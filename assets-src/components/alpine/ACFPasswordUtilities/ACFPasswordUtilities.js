...
export default ({} = {}) => ({
  ...
  init() {
    ...
    this.input = this.$el.closest('.acf-input').querySelector('.acf-input-wrap input')
    this.value = this.input.value
    this.input.addEventListener('input', e => this.onInput())
    this.$refs.toggle.addEventListener('click', e => this.onToggleClick(e))
    this.$refs.generator.addEventListener('click', e => this.onGeneratorClick(e))
  ...
  generatePassword() {
    const length = 12;
    const charset = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ01234567895!#.?";
    let result = "";
