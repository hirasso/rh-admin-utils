 ...
 */
function addCommitMessageToChangelog(
  pluginVersion,
  message,
  changelog = {},
  date = null
...
function getCommits() {
  ...
  const output = child
    .execSync(`git log --pretty=format:'%B###%H###%ad\n${delimiter}'`)
    .toString("utf-8");
...
function fileExistsInCommit(file, hash) {
  try {
    child.execSync(`git cat-file -e ${hash}:${file} > /dev/null 2>&1`);
    return true;
  } catch (e) {
    return false;
  }
...
async function generateChangelog() {
  ...
  for (const commit of Object.values(commitsArray)) {
    ...
    changelog = addCommitMessageToChangelog(
      getPluginVersion(pluginFile),
      message,
      changelog,
      date
    );
